...
void CumulativeDistributionFunction::BuildCDF(int nSteps) {
  ...
  for (int i = 0; i < nSteps; i += 2) {
    x = h * (i + 2);
    y += h / 3 *
         (Intensity(_fMin + i * h) + 4 * Intensity(_fMin + (i + 1) * h) +
          Intensity(_fMin + (i + 2) * h));
...
double CumulativeDistributionFunction::ProbFromVal(double fVal) {
  if (fVal < _fMin || fVal > _fMax) throw std::runtime_error("out of range");
...
double CumulativeDistributionFunction::ValFromProb(double fVal) {
  if (fVal < 0 || fVal > 1) throw std::runtime_error("out of range");
