...
const float OpenGLWnd::TimeStepSize = 100000.0f;
...
// OPENGL
void OpenGLWnd::InitGL() noexcept(false) {
  // GL initialization
...
void OpenGLWnd::UpdateText() {
  ...
  y += dy2;
  _textHelp.AddText(2, {x0, y}, "[+],[-] FoV:         %1.2f pc", _fov);
  ...
  _textHelp.AddText(1, {(float)_width - 180, (float)_height - 30},
                    " (C) 2020 Ingo Berg");
  _textHelp.EndUpdate();
...
// OPENGL
void OpenGLWnd::UpdateVelocityCurve() {
  ...
  float dt_in_sec = OpenGLWnd::TimeStepSize * Helper::SEC_PER_YEAR;
  float r = 0, v = 0;
 ...
 */
// OPENGL
void OpenGLWnd::UpdateDensityWaves() {
  std::vector<VertexColor> vert;
  std::vector<int> idx;
...
void OpenGLWnd::Update() {
  if (!(_flags & (int)DisplayItem::PAUSE)) _time += OpenGLWnd::TimeStepSize;

  if ((_renderUpdateHint & ruhDENSITY_WAVES) != 0) UpdateDensityWaves();

  if ((_renderUpdateHint & ruhAXIS) != 0) UpdateAxis();

  if ((_renderUpdateHint & ruhSTARS) != 0) UpdateStars();

  if ((_renderUpdateHint & ruhCREATE_VELOCITY_CURVE) != 0)
    UpdateVelocityCurve();

  if ((_renderUpdateHint & ruhCREATE_TEXT) != 0) UpdateText();
...
void OpenGLWnd::Render() {
  ...
  int features = 0;
  if (_flags & (int)DisplayItem::STARS) features |= 1 << 0;

  if (_flags & (int)DisplayItem::DUST) features |= 1 << 1;

  if (_flags & (int)DisplayItem::FILAMENTS) features |= 1 << 2;

  if (_flags & (int)DisplayItem::H2) features |= 1 << 3;

  if (features != 0) {
    _vertStars.UpdateShaderVariables(
        _time, _galaxy.GetPertN(), _galaxy.GetPertAmp(),
...
// OPENGL
void OpenGLWnd::AddEllipsisVertices(std::vector<VertexColor> &vert,
...
void OpenGLWnd::OnProcessEvents(Uint32 type) {
  switch (type) {
    ...
    case SDL_KEYDOWN:
      switch (_event.key.keysym.sym) {
        ...
        case SDLK_d:
          _galaxy.SetAngularOffset(
              std::max(_galaxy.GetAngularOffset() - 0.00002f, 0.0f));
          _renderUpdateHint |= ruhDENSITY_WAVES | ruhSTARS | ruhDUST;
          break;
        ...
        case SDLK_KP_0:
          ...
          _fov = 33960;
          _renderUpdateHint |=
              ruhDENSITY_WAVES | ruhSTARS | ruhDUST | ruhCREATE_VELOCITY_CURVE;
          break;
        ...
        case SDLK_KP_PLUS:
          ...
          _renderUpdateHint |=
              ruhAXIS |
              ruhDENSITY_WAVES;  // ruhDENSITY_WAVES only for the labels!
          break;
        ...
        case SDLK_ESCAPE:
          // This is a silly mechanism to disable sdl event polling and work
          // around an issue where the polling stops briefly evers 3 seconds.
          // the only purpose is to be able to screengrab proper animations.
          static int ct = 0;
          ct++;
          if (ct <= 5) {
            std::cout << "Stop pressing [ESC] or i will stop event polling!"
                      << std::endl;
          } else {
            std::cout << "Ok, you wanted this. Enjoy you frozen window..."
                      << std::endl;
