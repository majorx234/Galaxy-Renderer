...
void TextBuffer::Initialize() {
  ...
  _pSmallFont = TTF_OpenFont("/usr/share/fonts/TTF/Consolas-Regular.ttf", 14);
  if (_pSmallFont == nullptr) throw std::runtime_error(TTF_GetError());

  _pFont = TTF_OpenFont(
      "/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 18);
  if (_pFont == nullptr) throw std::runtime_error(TTF_GetError());

  _pFontCaption = TTF_OpenFont(
      "/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 40);
  if (_pFontCaption == nullptr) throw std::runtime_error(TTF_GetError());
...
void TextBuffer::Draw(int width, int height, glm::mat4& matView,
  ...
  for (int i = 0; i < _textureData.size(); ++i) {
    ...
    VertexTexture vertexData[6]{{x, y, 0, 0},
                                {x + w, y, 1, 0},
                                {x + w, y + h, 1, 1},
                                {x, y + h, 0, 1},
                                {x + w, y + h, 1, 1}};
    ...
    glVertexAttribPointer(attVertexPosition, 2, GL_FLOAT, GL_FALSE,
                          sizeof(VertexTexture), 0);

    // texture coord attribute
    glEnableVertexAttribArray(attTexturePosition);
    auto offset = offsetof(VertexTexture, tx);
    glVertexAttribPointer(attTexturePosition, 2, GL_FLOAT, GL_FALSE,
                          sizeof(VertexTexture), (GLvoid*)offset);

    glBindTexture(GL_TEXTURE_2D, td.id);

    GLuint projMatIdx = glGetUniformLocation(_shaderProgram, "projMat");

    // Note: You MUST use float parameters! (see
    // https://stackoverflow.com/questions/12230312/is-glmortho-actually-wrong)
    glm::mat4 projection = glm::ortho((float)0, (float)width, (float)height,
                                      (float)0, (float)0, (float)1);
    glUniformMatrix4fv(projMatIdx, 1, GL_FALSE, glm::value_ptr(projection));
    glBindVertexArray(vao);
    glDrawElements(GL_TRIANGLES, sizeof(vertexData) / sizeof(VertexTexture),
                   GL_UNSIGNED_INT, 0);
...
void TextBuffer::BeginUpdate() {
  ...
  if (_updating)
    throw std::runtime_error(
        "TextBuffer::BeginUpdate: An update is already in Progress!");
...
void TextBuffer::EndUpdate() {
  ...
  for (int i = 0; i < _textureData.size(); ++i) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)td.size.x,
                 (GLsizei)td.size.y, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 td.surface->pixels);
    CHECK_GL_ERROR
  }
...
void TextBuffer::AddText(int idxFont, glm::vec2 pos, const char* fmt, ...) {
  ...
  if (pSurface == nullptr)
    throw std::runtime_error(
        "TextBuffer::AddText: TTF_RenderText_Blended failed!");
  ...
  s = SDL_CreateRGBSurface(0, w, h, 32, 0x00ff0000, 0x0000ff00, 0x000000ff,
                           0xff000000);
  SDL_BlitSurface(pSurface, nullptr, s, nullptr);

  _textureData.push_back({s, pos, {(float)w, (float)h}});

  if (pSurface != nullptr) SDL_FreeSurface(pSurface);
