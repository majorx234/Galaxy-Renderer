...
bool Galaxy::HasDarkMatter() const noexcept { return _hasDarkMatter; }
...
void Galaxy::InitStarsAndDust() {
  ...
  for (int i = 0; i < _numStars; ++i) {
    ...
    dustParticle.velTheta =
        GetOrbitalVelocity((dustParticle.a + dustParticle.b) / 2.0f);
    dustParticle.type = 1;
  ...
  for (int i = 0; i < _numStars / 100; ++i) {
    ...
    for (int i = 0; i < num; ++i) {
      ...
      // I want the outer parts to appear blue, the inner parts yellow. I'm
      // imposing the following temperature distribution (no science here it
      // just looks right)
  ...
  for (int i = 0; i < _numH2; ++i) {
    ...
    particleH2.velTheta =
        GetOrbitalVelocity((particleH2.a + particleH2.b) / 2.0f);
...
float Galaxy::GetBaseTemp() const noexcept { return _baseTemp; }
...
const std::vector<Star>& Galaxy::GetStars() const { return _stars; }

float Galaxy::GetDustRenderSize() const { return _dustRenderSize; }

float Galaxy::GetRad() const { return _radGalaxy; }

float Galaxy::GetCoreRad() const { return _radCore; }

float Galaxy::GetFarFieldRad() const { return _radFarField; }
...
float Galaxy::GetExcentricity(float r) const {
  if (r < _radCore) {
    ...
    return _elEx1 +
           (r - _radCore) / (_radGalaxy - _radCore) * (_elEx2 - _elEx1);
  } else if (r > _radGalaxy && r < _radFarField) {
    // excentricity is slowly reduced to 1.
    return _elEx2 +
           (r - _radGalaxy) / (_radFarField - _radGalaxy) * (1 - _elEx2);
  } else
    return 1;
}

float Galaxy::GetAngularOffset(float rad) const { return rad * _angleOffset; }

float Galaxy::GetAngularOffset() const { return _angleOffset; }

float Galaxy::GetExInner() const { return _elEx1; }

float Galaxy::GetExOuter() const { return _elEx2; }

int Galaxy::GetPertN() const { return _pertN; }

float Galaxy::GetPertAmp() const { return _pertAmp; }
...
void Galaxy::SetPertAmp(float amp) { _pertAmp = std::max(0.0f, amp); }
