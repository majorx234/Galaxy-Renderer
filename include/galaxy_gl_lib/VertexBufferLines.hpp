...
class VertexBufferLines : public VertexBufferBase<VertexColor> {
 public:
  VertexBufferLines(int lineWidth = 1, GLuint bufferMode = GL_STATIC_DRAW)
      : VertexBufferBase(bufferMode), _lineWidth(lineWidth) {
    DefineAttributes({{attPosition, 3, GL_FLOAT, 0},
 ...
 protected:
  virtual const char* GetVertexShaderSource() const override {
    return "#version 330\n"
           ...
           "	vertexColor = color;\n"
           "	gl_PointSize = 3;\n"  // point size is only used for the
                                      // velocity curve
           "}\n";
  }
 ...
 private:
  enum AttributeIdx : int { attPosition = 0, attColor = 1 };
