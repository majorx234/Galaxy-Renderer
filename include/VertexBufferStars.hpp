...
class VertexBufferStars : public VertexBufferBase<VertexStar> {
 public:
  VertexBufferStars(GLuint blendEquation, GLuint blendFunc)
    ...
    DefineAttributes({{attTheta0, 1, GL_FLOAT, offsetof(Star, theta0)},
  ...
  virtual void Draw(glm::mat4& matView, glm::mat4& matProjection) {
    ...
    glDrawElements(GetPrimitiveType(), GetArrayElementCount(), GL_UNSIGNED_INT,
                   nullptr);
    glBindVertexArray(0);
 ...
 protected:
  virtual const char* GetVertexShaderSource() const override {
    return "#version 330 core\n"
           ...
           "\n"
           "vec2 calcPos(float a, float b, float theta, float velTheta, float "
           "time, float tiltAngle) {\n"
           ...
           "	vec2 ps = vec2(center.x + (a * cosalpha * cosbeta - b * "
           "sinalpha * sinbeta),\n"
           "			       center.y + (a * cosalpha * sinbeta + b "
           "* sinalpha * cosbeta));\n"
           ...
           "	    vertexColor = color * mag;\n"
           "	} else if (type==3) {\n"
           "		vec2 ps2 = calcPos(a + 1000.0, b, theta0, velTheta, "
           "time, tiltAngle);\n"
  ...
  virtual void OnSetCustomShaderVariables() override {
    ...
    GLuint varDisplayFeatures =
        glGetUniformLocation(GetShaderProgramm(), "displayFeatures");
